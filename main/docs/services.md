...
| --- | --- | --- |
| `power` | `vmxs5-embedded-power` | **Battery + charger state machine.** Sequences power-on/charging/shipping/sleep; talks to the charger IC and the Panasonic BMS. Rich state strings ("Charger connected", "Battery is fully charged, powering off", "going to shipping", reset/recovery handling). Publishes `power/state`, `power/battery/primary/info/soc`. |
| `ride` | `vmxs5-embedded-ride` | **Pedal-assist controller.** `RideService(IMQTTClient, IMotor, IPower, IPedalSensor, ITimerFactory, RideStrategy)`; uses an `SspProtocol`. `-t 1` selects a strategy/tuning. Subscribes `ride/#`, drives the motor. |
...
| `update` | `vmxs5-embedded-update` | **Peripheral OTA.** Given `/opt/devices_fw`, walks `manifest.txt` and flashes each sub-ECU over CAN/SPI; honors `AllowSkip`/`DontRollback`. `Type=notify`. Uses `vmxs5-embedded-motor-update-lib` + `vmxs5-embedded-lightweight-update`. |
